...
class KoodistotLoader:
    ...
    def get_object(
        ...
        if hasattr(code_class, "lifecycle_status_dict") and all_objects:
            statuses_with_code = [
                key
                for key, values in code_class.lifecycle_status_dict.items()
                if code_dict["value"] in values
            ]
            code_dict["lifecycle_status_ids"] = [
    ...
    def update_or_create_object(
        ...
        if "lifecycle_status_ids" in incoming.keys() and saved_objects:
            values["lifecycle_statuses"] = [
                saved_objects[id] for id in incoming["lifecycle_status_ids"]
            ]
