...
arho_count: int = 13  # adjust me when adding tables
...
def arho_database_created(root_db_params, main_db_params, current_head_version_id):
    event = {"event_type": 1}
...
@pytest.fixture()
def arho_database_migrated(root_db_params, main_db_params, current_head_version_id):
    event = {"event_type": 3}
    response = db_manager.handler(event, None)
    assert response["statusCode"] == 200, response["body"]
    yield current_head_version_id
...
@pytest.fixture()
def arho_database_migrated_down(arho_database_migrated):
...
def process_revision_directives_add_table(context, revision, directives):
    ...
    directives[0] = ops.MigrationScript(
        ...
        ops.DowngradeOps(
            ops=[ops.DropTableOp("test_table", schema="arho")],
        ),
    )
...
def autogenerated_migration(
    alembic_cfg, arho_database_migrated, current_head_version_id
):
...
@pytest.fixture()
def new_migration(alembic_cfg, arho_database_migrated, current_head_version_id):
    revision = command.revision(
        alembic_cfg,
        message="Test migration",
        head=current_head_version_id,
...
@pytest.fixture()
def arho_database_upgraded(new_migration):
    event = {"event_type": 3}
    response = db_manager.handler(event, None)
    assert response["statusCode"] == 200, response["body"]
    yield new_migration
...
@pytest.fixture()
def arho_database_downgraded(arho_database_upgraded, current_head_version_id):
...
def drop_arho_db(main_db_params, root_db_params):
...
def assert_database_is_alright(
    ...
    expected_arho_count: int = arho_count,
    ...
    # Check schemas
    cur.execute(
        "SELECT schema_name FROM information_schema.schemata WHERE schema_name IN ('arho', 'codes') ORDER BY schema_name DESC"  # noqa
    )
    assert cur.fetchall() == [("codes",), ("arho",)]

    # Check users
    arho_users = os.environ.get("DB_USERS", "").split(",")
    cur.execute("SELECT rolname FROM pg_roles")
    assert set(arho_users).issubset({row[0] for row in cur.fetchall()})

    # Check schema permissions
    for user in arho_users:
        cur.execute(f"SELECT has_schema_privilege('{user}', 'arho', 'usage')")
        assert cur.fetchall() == [(True,)]
        cur.execute(f"SELECT has_schema_privilege('{user}', 'codes', 'usage')")
        assert cur.fetchall() == [(True,)]

    # Check arho tables
    cur.execute("SELECT tablename, tableowner FROM pg_tables WHERE schemaname='arho';")
    arho_tables = cur.fetchall()
    assert len(arho_tables) == expected_arho_count
    ...
    for table in arho_tables:
        ...
        cur.execute(
            f"SELECT grantee, privilege_type FROM information_schema.role_table_grants WHERE table_schema = 'arho' AND table_name='{table_name}';"  # noqa
        )
        grants = cur.fetchall()
        ...
        # Check indexes
        cur.execute(
            f"SELECT * FROM pg_indexes WHERE schemaname = 'arho' AND tablename = '{table_name}';"  # noqa
        )
        indexes = cur.fetchall()
        cur.execute(
            f"SELECT column_name FROM information_schema.columns WHERE table_schema = 'arho' AND table_name = '{table_name}';"  # noqa
        )
        ...
        if ("id",) in columns:
            assert (
                ...
                f"{table_name}_pkey",
                None,
                f"CREATE UNIQUE INDEX {table_name}_pkey ON arho.{table_name} USING btree (id)",  # noqa
            ) in indexes
        if ("geom",) in columns:
            assert (
                "arho",
                ...
                None,
                f"CREATE INDEX idx_{table_name}_geom ON arho.{table_name} USING gist (geom)",  # noqa
            ) in indexes
        if ("ordering",) in columns:
            assert (
                "arho",
                table_name,
                f"ix_arho_{table_name}_ordering",
                None,
                f"CREATE INDEX ix_arho_{table_name}_ordering ON arho.{table_name} USING btree (ordering)",  # noqa
            ) in indexes
...
@pytest.fixture(scope="module")
def connection_string(arho_database_created) -> str:
