...
@pytest.fixture()
def type_of_plan_regulation_floor_area_ratio_instance(session):
    instance = codes.TypeOfPlanRegulation(value="tehokkuusluku", status="LOCAL")
    session.add(instance)
    session.commit()
    yield instance
    session.delete(instance)
...
# Therefore, decimal plan regulation requires its own group.
@pytest.fixture(scope="function")
...
def decimal_plan_regulation_instance(
    session,
    preparation_status_instance,
    type_of_plan_regulation_floor_area_ratio_instance,
    decimal_plan_regulation_group_instance,
):
    instance = models.PlanRegulation(
        subject_identifiers=["#test_regulation"],
        ...
        unit="m",
        lifecycle_status=preparation_status_instance,
        type_of_plan_regulation=type_of_plan_regulation_floor_area_ratio_instance,
...
def complete_test_plan(
    ...
    # decimal cannot be used with intended use regulation type
    ...
    # verbal cannot be used with intended use regulation type
    general_plan_regulation_instance.plan_theme = plan_theme_instance
    # general cannot be used with intended use regulation type
...
def participation_plan_presenting_for_public_decision(
    ...
    instance = codes.NameOfPlanCaseDecision(
        value="04", status="LOCAL", lifecycle_statuses=[preparation_status_instance]
    )
    session.add(instance)
    session.commit()
    yield instance
...
def plan_material_presenting_for_public_decision(session, preparation_status_instance):
    instance = codes.NameOfPlanCaseDecision(
        value="05", status="LOCAL", lifecycle_statuses=[preparation_status_instance]
    )
    session.add(instance)
    session.commit()
    yield instance
...
def draft_plan_presenting_for_public_decision(session, preparation_status_instance):
    instance = codes.NameOfPlanCaseDecision(
        value="06", status="LOCAL", lifecycle_statuses=[preparation_status_instance]
    )
    session.add(instance)
    session.commit()
    yield instance
...
def plan_proposal_sending_out_for_opinions_decision(
    ...
    instance = codes.NameOfPlanCaseDecision(
        value="07", status="LOCAL", lifecycle_statuses=[plan_proposal_status_instance]
    )
    session.add(instance)
    session.commit()
    yield instance
...
def plan_proposal_presenting_for_public_decision(
    ...
    instance = codes.NameOfPlanCaseDecision(
        value="08", status="LOCAL", lifecycle_statuses=[plan_proposal_status_instance]
    )
    session.add(instance)
    session.commit()
    yield instance
...
def presentation_to_the_public_interaction(
    ...
    instance = codes.TypeOfInteractionEvent(
        value="01",
        status="LOCAL",
        lifecycle_statuses=[preparation_status_instance, plan_proposal_status_instance],
    )
    session.add(instance)
    session.commit()
    yield instance
                ...
                "planRegulations": [
                    {
                        ...
                        "lifeCycleStatus": "http://uri.suomi.fi/codelist/rytj/kaavaelinkaari/code/03",
                        "type": "http://uri.suomi.fi/codelist/rytj/RY_Kaavamaarayslaji/code/tehokkuusluku",
                        "value": {
                            "dataType": "positiveDecimal",
                            "number": decimal_plan_regulation_instance.decimal_value,
